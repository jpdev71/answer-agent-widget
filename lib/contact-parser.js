...
function parseContact(text) {
  ...
  const leadingName = explicitName || standaloneName || nearContactName ? "" : findLeadingName(source);

  return {
    name: sanitizeDetectedName(explicitName || standaloneName || nearContactName || leadingName),
    phone,
    email,
  };
}
...
function findLeadingName(text) {
  const match = text.match(/^\s*([A-Za-z][A-Za-z'’-]*(?:\s+[A-Za-z][A-Za-z'’-]*){0,3})\s*[,.]/i);
  return match && looksLikeHumanName(match[1]) ? match[1] : "";
}
...
function looksLikeHumanName(candidate) {
  ...
  return tokens.every((token) => {
    ...
    return /^[a-z][a-z'’-]*$/i.test(token) && !NON_NAME_WORDS.has(normalized);
  });
}
...
function looksLikeStandaloneNameLine(line) {
  ...
  return looksLikeHumanName(cleaned);
}
